...
export const router_video = Router({})
...
router_video.get('/', (req: Request, res: Response) => {
  res.status(200).send(videos)
})

router_video.get('/:id', (req: Request, res: Response) => {
...
router_video.delete('/:id', (req: Request, res: Response) => {
...
router_video.post('/', (req: Request, res: Response) => {
  let body = req.body
...
router_video.put('/:id', (req: Request, res: Response) => {
  let body = req.body
...
function FieldsValidation(body: any): boolean {
  ...
  if (
    body.availableResolutions !== undefined &&
    !ResolutionsIsValid(body.availableResolutions)
  ) {
    AddMessage(
      'availableResolutions',
      'inputModel [availableResolutions] has incorrect values',
    )
    validationPassed = false
  }
  ...
  if (
    ...
    AddMessage(
      'canBeDownloaded',
      'inputModel [canBeDownloaded] has incorrect values',
    )
    validationPassed = false
  }

  if (
    (body.minAgeRestriction !== undefined &&
    ...
    AddMessage(
      'minAgeRestriction',
      'inputModel [minAgeRestriction] has incorrect values',
    )
    validationPassed = false
  }
  ...
  if (
    ...
    AddMessage(
      'publicationDate',
      'inputModel [publicationDate] has incorrect values',
    )
    validationPassed = false
  }
